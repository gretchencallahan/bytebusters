...
public class AppraisalController {
    ...
    private serviceOneAppraisal appraisalService = new serviceOneAppraisal();
    ...
    public ResponseEntity<ArrayList<String>> getAppraisals() {
        ...
        for (HouseObj houseObj : appraisals.values()){
            list.add("http://localhost:5050/appraisals/" + houseObj.houseName);
        }
    ...
    public ResponseEntity<HouseObj> createBasePrice(@RequestBody HouseObj house) {
            HouseObj houseObj = appraisalService.generateBasePrice(house);
            appraisals.put(houseObj.houseName, houseObj);
            String url = "http://" + "localhost:5050/appraisals/" + houseObj.houseName;
