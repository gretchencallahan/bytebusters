...
public class AppraisalController {
    ...
    private serviceThreeAppraisal appraisalService = new serviceThreeAppraisal();
    ...
    public ResponseEntity<ArrayList<String>> getAppraisals() {
        ...
        for (HouseObj houseObj : appraisals.values()){
            list.add("http://localhost:5052/appraisals/" + houseObj.houseName);
        }
    ...
    public ResponseEntity<HouseObj> createBasePrice(@RequestBody HouseObj house) {
            HouseObj houseObj = appraisalService.checkLocation(house);
            appraisals.put(houseObj.houseName, houseObj);
            String url = "http://" + "localhost:5052/appraisals/" + houseObj.houseName;
