...
public class AppraisalController {
    ...
    private serviceTwoAppraisal appraisalService = new serviceTwoAppraisal();
    ...
    public ResponseEntity<ArrayList<String>> getAppraisals() {
        ...
        for (HouseObj houseObj : appraisals.values()){
            list.add("http://localhost:5051/appraisals/" + houseObj.houseName);
        }
    ...
    public ResponseEntity<HouseObj> createBasePrice(@RequestBody HouseObj house) {
            HouseObj houseObj = appraisalService.checkHistory(house);
            appraisals.put(houseObj.houseName, houseObj);
            String url = "http://" + "localhost:5051/appraisals/" + houseObj.houseName;
